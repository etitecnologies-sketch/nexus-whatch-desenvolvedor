...
// Supabase client for backend (with service role key to bypass RLS if needed)
...
class SDKServer {
  ...
  async verifySession(
  ...
  ): Promise<{ supabaseId: string; name: string } | null> {
    if (!cookieValue) {
      return null;
    }
    ...
    try {
      ...
      const { supabaseId, name } = payload as Record<string, unknown>;

      if (!isNonEmptyString(supabaseId) || !isNonEmptyString(name)) {
        return null;
      }
